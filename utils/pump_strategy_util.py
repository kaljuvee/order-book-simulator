...
def fetch_top_movers(exchange, selected_interval, total_limit=50, top_limit=20):
    try:
        start_time = datetime.now()
        top_symbols = exchange_util.get_top_symbols_by_volume_usd(exchange, total_limit)
        top_movers = exchange_util.get_top_price_movers(exchange, top_symbols, selected_interval, '1m', top_limit)
    
        end_time = datetime.now()
        duration = end_time - start_time
        minutes, seconds = divmod(duration.seconds, 60)
        print(f"fetch_top_movers: got top movers in {minutes} minutes and {seconds} seconds.")
        
        return top_movers
    except Exception as e:
        print(f"fetch_top_movers: error fetching top movers: {e}")
...
def check_exit_signal(latest_price, position_entry_price, volume_ratio, profit_target, stop_loss):
    ...
    try:
        ...
        elif percent_change_from_entry < stop_loss: 
            exit_signal = True
            exit_reason = 'profit_target'
